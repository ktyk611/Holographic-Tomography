...
# --- 複素投影から Sinogram作成
# img_z:投影角数
img_z, img_y, img_x = complex_proj.shape

# --- 複素振幅分布（生のホログラム）によるSinogram
sinogram_complex = np.zeros((img_y,img_z,img_x),dtype="complex128")
for i in range (img_y):
    for j in range (img_z):
        sinogram_complex[i,j,:] = complex_proj[j,i,:]
np.save(r"sinogram/sinogram_complex.npy",sinogram_complex)

plt.subplot(231,title="0");plt.imshow(np.angle(complex_proj[0]),"hsv");plt.colorbar(label='phase')
plt.subplot(232,title="45");plt.imshow(np.angle(complex_proj[30]),"hsv");plt.colorbar(label='phase')
plt.subplot(233,title="90");plt.imshow(np.angle(complex_proj[59]),"hsv");plt.colorbar(label='phase')
plt.subplot(234,title="0");plt.imshow(np.abs(complex_proj[0]),"viridis");plt.colorbar(label='Amplitude')
plt.subplot(235,title="45");plt.imshow(np.abs(complex_proj[30]),"viridis");plt.colorbar(label='Amplitude')
plt.subplot(236,title="90");plt.imshow(np.abs(complex_proj[59]),"viridis");plt.colorbar(label='Amplitude')
plt.suptitle(f"Complex Projection")
plt.show()
